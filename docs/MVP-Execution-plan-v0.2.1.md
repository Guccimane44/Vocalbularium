...
### 2. Render dashboard capture feedback in its own page — medium
...
- Reuse the existing feedback presentation in that page, mounted outside the account content replaced during dashboard rendering. A capture/account refresh must not immediately erase its feedback.
...
- Keep normal webpage overlays and the genuine restricted-page fallback functioning. Use the existing permissions; the dashboard fix should not require broader website access.
- Clarify the dashboard feedback behavior in [Select and Add](MVP-Product-spec-select-and-add.md#capture-feedback-popup), keeping its detailed capture, retry and interruption rules authoritative.

**Exit:** dashboard capture shows one in-page receipt, saves the card normally, preserves focus/route, and creates zero additional Chrome windows.
...
- Hand over the package for an in-place Windows update. Record Windows/Chrome versions, source/checksum, removal of the texts, and absence of the blank-window flash during repeated dashboard captures in both themes.
