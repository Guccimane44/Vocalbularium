...
For this owner testing phase, use OpenCode `deepseek-v4.1-flash` and a Render Free web service, as recorded in the [implementation plan](MVP-Implementation-plan.md). Its sleeping service and ephemeral storage are accepted testing constraints: server sleep, restart, or redeploy can reset the entire test account. Synchronization applies while that account exists. Persistent hosted storage and always-on responsiveness remain open requirements for production use; this exception does not establish persistence acceptance. Other saving, generation, and retry behavior remains governed by [Select and Add](MVP-Product-spec-select-and-add.md).
