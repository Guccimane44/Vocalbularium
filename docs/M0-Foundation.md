...
| Generation | Updated for owner testing: OpenCode Zen Chat Completions with `mimo-v2.5-free`, configurable through `OPENCODE_MODEL`. Prompt for JSON and validate it locally before staging page output; no automatic model fallback. Live access is pending the owner's OpenCode key. See [OpenCode Zen](https://opencode.ai/docs/zen/). |
